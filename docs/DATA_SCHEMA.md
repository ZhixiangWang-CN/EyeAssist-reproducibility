...
| `patient_id` | string | ✓ | stable de-identified patient key; multiple `case_id` values may map to one patient |
...
Patient-disjoint analyses use `patient_id` as the grouping field. The EyeAssist-Neo manifest contains
75 image-case records from 68 patients, so `patient_id`, rather than `case_id`, defines independence.
